    ...
    for sq in &plan.sub_queries {
        steps.push(PlanStep {
            ...
            detail: format!(
                "Signal '{}' → {} → backend '{}' ({})",
                sq.signal_type,
                sq.transpiler_name.to_uppercase(),
                sq.backend_name,
                sq.backend_url
            ),
...
mod tests {
    ...
    async fn explain_has_native_query_in_transpile_step() {
        let resp = explain_query("FROM metrics WHERE __name__ = \"up\"").await;
        let transpile_step = resp
            .plan
            .steps
            .iter()
            .find(|s| s.action.starts_with("transpile_"));
    ...
    async fn explain_correlate_has_decompose_and_correlate_steps() {
        ...
        assert!(
            decompose.is_some(),
            "Should have a decompose step for multi-signal"
        );
