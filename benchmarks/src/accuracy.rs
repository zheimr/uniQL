...
async fn main() {
    ...
    println!("╚══════════════════════════════════════════════════════════════════╝");
    println!();
    println!("  Corpus: {} queries ({} simple, {} realistic, {} stress)",
        queries.len(),
        queries.iter().filter(|q| q.tier == corpus::Tier::Simple).count(),
        queries.iter().filter(|q| q.tier == corpus::Tier::Realistic).count(),
        queries.iter().filter(|q| q.tier == corpus::Tier::Stress).count(),
    );
    ...
    for q in &queries {
        ...
        println!("  [{:8}] {:25} {:>5}ms vs {:>5}ms  cnt: {:>4} vs {:>4}  {}",
    ...
    let any_match = results.iter().filter(|r| r.exact_match || r.sorted_match || r.float_tolerance_match).count();

    println!("  Transpile success:       {:>3}/{} ({:.1}%)", transpile_ok, total, transpile_ok as f64 / total as f64 * 100.0);
    println!("  Execute success:         {:>3}/{} ({:.1}%)", execute_ok, total, execute_ok as f64 / total as f64 * 100.0);
    println!("  Exact match:             {:>3}/{} ({:.1}%)", exact, total, exact as f64 / total as f64 * 100.0);
    println!("  Sorted equivalence:      {:>3}/{} ({:.1}%)", sorted, total, sorted as f64 / total as f64 * 100.0);
    println!("  Float tolerance match:   {:>3}/{} ({:.1}%)", float_tol, total, float_tol as f64 / total as f64 * 100.0);
    println!("  Any match (semantic eq): {:>3}/{} ({:.1}%)", any_match, total, any_match as f64 / total as f64 * 100.0);
    println!();

    // By tier
    for tier in [corpus::Tier::Simple, corpus::Tier::Realistic, corpus::Tier::Stress] {
        let tier_results: Vec<_> = results.iter().filter(|r| r.tier == tier.to_string()).collect();
        let t = tier_results.len();
        if t == 0 { continue; }
        let m = tier_results.iter().filter(|r| r.exact_match || r.sorted_match || r.float_tolerance_match).count();
        println!("  {:10} {}/{} semantic match ({:.1}%)", tier.to_string().to_uppercase(), m, t, m as f64 / t as f64 * 100.0);
    }

    // Latency summary
    println!();
    let uniql_latencies: Vec<u64> = results.iter().filter(|r| r.execute_ok).map(|r| r.uniql_ms).collect();
    let direct_latencies: Vec<u64> = results.iter().filter(|r| r.direct_ok).map(|r| r.direct_ms).collect();

    if !uniql_latencies.is_empty() {
        println!("  UNIQL latency:  p50={}ms  p95={}ms  p99={}ms",
    ...
    if !direct_latencies.is_empty() {
        println!("  Direct latency: p50={}ms  p95={}ms  p99={}ms",
    ...
    if !uniql_latencies.is_empty() && !direct_latencies.is_empty() {
        let uniql_avg: f64 = uniql_latencies.iter().sum::<u64>() as f64 / uniql_latencies.len() as f64;
        let direct_avg: f64 = direct_latencies.iter().sum::<u64>() as f64 / direct_latencies.len() as f64;
        let overhead = uniql_avg - direct_avg;
        println!("  UNIQL overhead: {:.1}ms avg ({:.1}%)", overhead, if direct_avg > 0.0 { overhead / direct_avg * 100.0 } else { 0.0 });
    }
...
async fn test_query(client: &reqwest::Client, q: &corpus::BenchQuery) -> AccuracyResult {
    ...
    let uniql_resp = client.post(format!("{}/v1/query", ENGINE_URL))
    ...
    let direct_data = match q.expected_backend {
        "promql" => {
            let resp = client.get(format!("{}/api/v1/query", VM_URL))
        ...
        "logsql" => {
            let resp = client.get(format!("{}/select/logsql/query", VL_URL))
    ...
    let direct_data = match direct_data {
        Some(d) => { result.direct_ok = true; d }
        None => {
    ...
    match q.expected_backend {
        "promql" => {
            ...
            if !result.exact_match && !result.sorted_match {
                result.float_tolerance_match = float_compare(&uniql_results, &direct_results, 0.001);
            }
        }
        "logsql" => {
...
fn extract_prom_values(data: &serde_json::Value) -> Vec<String> {
    ...
    if let Some(results) = data.get("data").and_then(|d| d.get("result")).and_then(|r| r.as_array()) {
        for item in results {
